...
async def add_image(book_id: str, image_file: UploadFile = File(...)):
    ...
    image_name = image_name[0:image_point] + str(int(book_id) + 1) + image_name[image_point:len(image_name)]
    file_type = image_file.content_type
    if "image" in file_type:
        with open(f"{path}/{image_name}", "wb") as image:
            copyfileobj(image_file.file, image)
        image_dict = {"name": image_name, "size": image_file.size, "file_type": file_type, "book_id": book_id + 1}
...
# Удаление изоюражения
async def delete_image(image_name: str, book_id: int):
